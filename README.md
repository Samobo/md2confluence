...
`md2confluence` is a tool that allows you to convert Markdown documents to Confluence Storage format and upload them to a Confluence instance.

The code base is from [md2cf@2.3.0](https://github.com/iamjackg/md2cf/tree/v2.3.0) by [Jack Gaino](https://github.com/iamjackg). Thanks for his work.  
Calls to the Confluence API have been adapted to use both Confluence API V1 (/rest/api/) and V2 (/api/v2/), depending on the endpoint.
...
Then run md2cf with `uv run md2cf <args>`.

Alternatively, you can use the provided Dockerfile to run the tool in a container.  
First build the image with `docker build -t md2confluence .`,  
Then run a container using this image like `docker run --rm md2confluence <args>`.


## Getting started

To see all available options and parameters, run `uv run md2cf --help`.

To upload a document, you need to provide at least the following five parameters:

- The **URL** of your Confluence instance (without the path to the REST API, e.g. `http://confluence.example.com`)
- Either:
