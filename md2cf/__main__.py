...
from tui import Md2cfTUI

from upsert import upsert_attachment, upsert_page
...
def main():
    ...
    if args.preface_markdown:
        preface_markup = document.parse_page([args.preface_markdown]).body
    elif args.preface_file:
        # We don't use strip_header or remove_text_newlines here
        # since this is just a preface doc
        preface_markup = document.get_page_data_from_file_path(args.preface_file).body

    postface_markup = ""
    if args.postface_markdown:
        postface_markup = document.parse_page([args.postface_markdown]).body
    elif args.postface_file:
        # We don't use strip_header or remove_text_newlines here
        # since this is just a postface doc
        postface_markup = document.get_page_data_from_file_path(args.postface_file).body
...
def collect_pages_to_upload(args):
    ...
    if not args.file_list:  # Uploading from standard input
        pages_to_upload.append(
            document.get_page_data_from_lines(
                sys.stdin.readlines(),
        ...
        for file_name in args.file_list:
            if file_name.is_dir():
                pages_to_upload += document.get_pages_from_directory(
                    file_name,
                ...
                try:
                    ...
                    pages_to_upload.append(
                        document.get_page_data_from_file_path(
                            file_name,
                            strip_header=args.strip_top_header,
                            remove_text_newlines=args.remove_text_newlines,
